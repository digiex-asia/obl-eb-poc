...
type ElementNode = {
    type: 'paragraph' | 'heading-1' | 'heading-2' | 'bullet-list' | 'numbered-list';
...
const WEB_SAFE_FONTS = ['Arial', 'Verdana', 'Times New Roman', 'Georgia', 'Courier New', 'Impact'];
const GOOGLE_FONTS = ['Roboto', 'Open Sans', 'Lato', 'Montserrat', 'Playfair Display', 'Oswald'];

type PluginProps = {
    currentStyle: Partial<TextLeaf> & { blockAnimation?: BlockAnimation; lineHeight?: number };
...
const Editor = {
    isCollapsed: (range: Range) =>
        range.anchor.path[0] === range.focus.path[0] && range.anchor.offset === range.focus.offset,
...
const Transforms = {
    ...
    deleteBackward: (content: ElementNode[], at: Range) => {
        ...
        if (start.offset !== end.offset) {
            ...
            if (newChildren.length === 0) newChildren.push({ ...node.children[0], text: '' });
    ...
    applyStyleToRange: (content: ElementNode[], style: any, at: Range) => {
        ...
        if ('blockAnimation' in style || 'lineHeight' in style || 'align' in style) {
        ...
        for (const leaf of node.children) {
            ...
            } else if (Math.max(leafStart, start.offset) < Math.min(leafEnd, end.offset)) {
...
export default function App() {
    ...
    const [dragMode, setDragMode] = useState<'none' | 'select' | 'move-box' | ResizeHandle>('none');
    ...
    const layoutMap = useRef<Record<string, { lines: RenderedLine[]; height: number }>>({});
    ...
    const computeBoxLayout = (ctx: CanvasRenderingContext2D, box: TextBox) => {
        ...
        box.content.forEach((node, nodeIndex) => {
            ...
            const flushLine = () => {
                let startX = PADDING;
                if (node.align === 'center') startX += (maxWidth - currentLineWidth) / 2;
            ...
            node.children.forEach((leaf, leafIndex) => {
                ...
                chars.forEach((char, charIdx) => {
                    ...
                    if (currentLineWidth + w > effectiveMaxWidth && currentLineChars.length > 0)
                        flushLine();
        ...
        textBoxes.forEach((box) => {
            ...
            if (box.id === activeBoxId) {
                ...
                const handles = [
                    ...
                    { x: box.width - HANDLE_SIZE / 2, y: actualBoxHeight / 2 - HANDLE_SIZE / 2 }, // E
                    { x: box.width - HANDLE_SIZE / 2, y: actualBoxHeight - HANDLE_SIZE / 2 }, // SE
                    { x: box.width / 2 - HANDLE_SIZE / 2, y: actualBoxHeight - HANDLE_SIZE / 2 }, // S
            ...
            layout.lines.forEach((line) => {
                ...
                line.chars.forEach((charData) => {
                    ...
                    if (s.animation === 'wavy')
                        ...
                        drawSize = drawSize * (1 + Math.sin(time / 200 + absCharIndex) * 0.15);
                    ...
                    const textY = line.y + (line.height - drawSize) / 2 + lineOffsetY + drawOffsetY;
            ...
            const cursorVisible = time % 1000 < 500 || time - lastInputTime.current < 500;
                ...
                for (const line of layout.lines) {
                    ...
                    if (line.chars.length === 0) {
                        ...
                        if (focus.offset === 0) {
                            ...
                            ctx.lineTo(PADDING + lineOffsetX, line.y + line.height + lineOffsetY);
                            ctx.lineWidth = 2;
                    ...
                    if (
                        ...
                        if (localOffset > 0 && line.chars[localOffset - 1]) {
                            cx = line.chars[localOffset - 1].x + line.chars[localOffset - 1].width;
                        }
    ...
    const getResizeHandle = (box: TextBox, mx: number, my: number): ResizeHandle | null => {
        const computedHeight = layoutMap.current[box.id]?.height || 50;
        ...
        if (lx >= w - hs && lx <= w + hs && ly >= h - hs && ly <= h + hs) return 'se';
        if (lx >= -hs && lx <= hs && ly >= h - hs && ly <= h + hs) return 'sw';
        if (lx >= w / 2 - hs && lx <= w / 2 + hs && ly >= -hs && ly <= hs) return 'n';
        if (lx >= w / 2 - hs && lx <= w / 2 + hs && ly >= h - hs && ly <= h + hs) return 's';
        if (lx >= -hs && lx <= hs && ly >= h / 2 - hs && ly <= h / 2 + hs) return 'w';
        if (lx >= w - hs && lx <= w + hs && ly >= h / 2 - hs && ly <= h / 2 + hs) return 'e';
    ...
    const hitTestChar = (box: TextBox, mx: number, my: number): Point | null => {
        ...
        if (rx > lastChar.x + lastChar.width) return getGlobalOffset(box, line, lastChar, 1);
    ...
    const getGlobalOffset = (
        ...
        let abs = 0;
        const node = box.content[line.path[0]];
        for (let k = 0; k < char.leafIndex; k++) abs += node.children[k].text.length;
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        ...
        if (dragMode === 'move-box') {
            ...
            setTextBoxes((p) =>
                p.map((b) =>
                    b.id === activeBoxId ? { ...b, x: newX, y: newY, width: newW, height: newH } : b
                )
            );
        }
    };
    ...
    const handleDoubleClick = (e: React.MouseEvent) => {
        if (!activeBoxId && canvasRef.current) {
            ...
            setSelection({ anchor: { path: [0], offset: 0 }, focus: { path: [0], offset: 12 } });
        }
    };
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
        ...
        setTextBoxes((prev) => {
            ...
            if (key === 'Backspace') {
                ...
                newSel = {
                    ...selection,
                    focus: { ...selection.focus, offset: Math.max(0, selection.focus.offset - 1) },
                };
