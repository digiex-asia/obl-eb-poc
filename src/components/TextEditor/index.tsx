...
import { MainToolbar, ElementToolbar } from './Toolbar';
...
export default function CanvasRichTextEditor() {
  ...
  // Track viewport size changes for toolbar positioning
  useEffect(() => {
    const handleResize = () => {
    ...
    elements.forEach(el => {
      ...
      if (selectedId === el.id) {
        ...
        if (!isEditing) {
          ...
          const anchors = [
            { x: 0, y: 0 }, // top-left
            { x: el.width / 2, y: 0 }, // top-center
            { x: el.width, y: 0 }, // top-right
            { x: el.width, y: boxHeight / 2 }, // right-center
            { x: el.width, y: boxHeight }, // bottom-right
            { x: el.width / 2, y: boxHeight }, // bottom-center
          ...
          anchors.forEach((anchor, index) => {
            if (el.autoFit) {
              ...
              // Hide top-center (index 1) and bottom-center (index 5)
              if (index !== 1 && index !== 5) {
                drawAnchor(anchor.x, anchor.y);
              }
            }
    ...
    if (selectedId && !isEditing) {
      ...
      if (el) {
        ...
        const anchors = [
          ...
          { x: el.x + el.width / 2, y: renderY, type: 'resize_t' }, // top-center
          { x: el.x + el.width, y: renderY, type: 'resize_tr' }, // top-right
          { x: el.x + el.width, y: renderY + boxHeight / 2, type: 'resize_r' }, // right-center
          { x: el.x + el.width, y: renderY + boxHeight, type: 'resize_br' }, // bottom-right
          { x: el.x + el.width / 2, y: renderY + boxHeight, type: 'resize_b' }, // bottom-center
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (dragInfo.isDragging && activeElement) {
      ...
      if (dragInfo.type === 'move') {
        ...
        switch (resizeType) {
          case 'resize_tl': // top-left: resize both, move x and y
            newWidth = Math.max(50, origWidth - dx);
            newHeight = Math.max(50, origHeight - dy);
            newX = origX + (origWidth - newWidth);
            newY = origY + (origHeight - newHeight);
            break;
          case 'resize_t': // top-center: resize height only, move y (only when autoFit)
            newHeight = Math.max(50, origHeight - dy);
            newY = origY + (origHeight - newHeight);
            break;
          case 'resize_tr': // top-right: resize both, move y
            newWidth = Math.max(50, origWidth + dx);
            newHeight = Math.max(50, origHeight - dy);
            newY = origY + (origHeight - newHeight);
            break;
          case 'resize_r': // right-center: resize width only
            newWidth = Math.max(50, origWidth + dx);
            break;
          case 'resize_br': // bottom-right: resize both
            newWidth = Math.max(50, origWidth + dx);
            newHeight = Math.max(50, origHeight + dy);
            break;
          case 'resize_b': // bottom-center: resize height only (only when autoFit)
            newHeight = Math.max(50, origHeight + dy);
            break;
          case 'resize_bl': // bottom-left: resize both, move x
            newWidth = Math.max(50, origWidth - dx);
            newHeight = Math.max(50, origHeight + dy);
            newX = origX + (origWidth - newWidth);
            break;
          case 'resize_l': // left-center: resize width only, move x
            newWidth = Math.max(50, origWidth - dx);
            newX = origX + (origWidth - newWidth);
            break;
        }
  ...
  // Calculate toolbar position for active element
  ...
  const getToolbarPosition = useMemo(() => {
    ...
    if (toolbarViewportY - TOOLBAR_ESTIMATED_HEIGHT < viewportTop) {
      ...
      const boxHeight = activeElement.autoFit ? activeElement.height : layoutH;
    ...
    if (toolbarViewportY + TOOLBAR_ESTIMATED_HEIGHT > viewportBottom) {
      ...
      // If we still can't fit, try above even if it's tight
      if (toolbarViewportY - TOOLBAR_ESTIMATED_HEIGHT < viewportTop) {
        toolbarViewportY = viewportTop;
        positionAbove = true;
      }
    }
        ...
        <div ref={canvasContainerRef} className="relative shadow-2xl bg-white ring-1 ring-gray-900/5" style={{ width: STAGE_WIDTH, height: STAGE_HEIGHT }}>
          ...
          {activeElement && getToolbarPosition && (
            <ElementToolbar
              ...
              positionAbove={getToolbarPosition.positionAbove}
            />
          )}
        </div>
      </div>
